...
func (c *Core) retrySingleTokenDownload(download *model.FailedFTDownload) error {
	...
	ftTokenHash, err := c.ipfsOps.Add(bytes.NewBufferString(download.TokenID), ipfsnode.Pin(false), ipfsnode.OnlyHash(true))
	if err != nil {
		return fmt.Errorf("retrySingleTokenDownload: Unable to do IPFS Add operation on Token: %v", err)
	}

	err = c.w.GetIpfsOps().Get(ftTokenHash, download.TokenPath)
	if err != nil {
