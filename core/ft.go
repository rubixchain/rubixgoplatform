...
func (c *Core) updateFTTable() error {
	...
	if err != nil {
		...
		if strings.Contains(fetchErr, "no records found") {
			// If no FT source records found, clear the FT table
			err = c.s.Delete(wallet.FTStorage, &wallet.FT{}, "ft_name IS NOT NULL")
			if err != nil {
	...
	// Clear existing FT records before rebuilding (fixed logic)
	err = c.s.Delete(wallet.FTStorage, &wallet.FT{}, "ft_name IS NOT NULL")
	if err != nil {
