...
func performTokenSplit(w *wallet.Wallet, dc types.DIDCrypto,
	...
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("performTokenSplit: failed to get denom level for parent token %s; err: %w", parentTokenID, err)
	}
	...
	return freeTokens, keepTokens, burnTokens, childMintRecords, nil
}
...
func createChildTokensAtLevel(dc types.DIDCrypto, w *wallet.Wallet, parentTokenID TokenID,
	...
	for index := childrenIndexRange.First; index <= childrenIndexRange.Last; index++ {
		...
		// The key in the map represents child position among all children (1-based).
