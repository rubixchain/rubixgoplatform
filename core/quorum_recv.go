...
func (c *Core) getProofverificationDetails(tokenID string, senderAddr string) (string, string, error) {
	...
	tokenPledgedForDetails := prevBlk.GetTokenPledgedForDetails()
	if tokenPledgedForDetails == "" {
	...
	tpfdArray := strings.Split(tokenPledgedForDetails, ",")

	tokenPledgedFor := tpfdArray[0]
	tokenPledgedForTypeStr := tpfdArray[1]
	tokenPledgedForBlock := tpfdArray[2]
	...
	pledgedfBlk, err := c.w.GetTokenBlock(tokenPledgedFor, tokenPledgedForType, tokenPledgedForBlock)
	if err != nil {
	...
	if pledgedfBlk != nil {
		...
		err = c.syncTokenChainFrom(p, tokenPledgedForBlock, tokenPledgedFor, tokenPledgedForType)
		if err != nil {
			c.log.Error("Failed to sync token chain block", "err", err)
			return "", "", err
		}
		tcbArray, err := c.w.GetTokenBlock(tokenPledgedFor, tokenPledgedForType, tokenPledgedForBlock)
		if err != nil {
			c.log.Error("Failed to fetch previous block", "err", err)
			return "", "", err
		}
	...
	return pledgedforBlk.GetReceiverDID(), pledgedforBlk.GetTid(), nil
}
