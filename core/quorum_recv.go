...
func (c *Core) quorumFTConsensus(req *ensweb.Request, did string, qdc didcrypto.DIDCrypto, cr *ConensusRequest) *ensweb.Result {
	...
	for i := range ti {
		wg.Add(1)
		go c.pinCheck(ti[i].Token, i, cr.SenderPeerID, cr.ReceiverPeerID, results, &wg, nil)
	}
	wg.Wait()
	for i := range results {
		if results[i].Error != nil {
...
func (c *Core) updateReceiverToken(
	...
	if receiverAddress != "" {
		...
		for _, ti := range tokenInfo {
			...
			if parentTokenPart != "" {
				_, err := c.w.Pin(parentTokenPart, wallet.ParentTokenLockRole, senderAddress, b.GetTid(), senderAddress, receiverAddress, ti.TokenValue)
				if err != nil {
			...
			err = c.validateLatestBlock(senderPeer, ti, receiverDID, senderDid, parentTokenPart, b)
			if err != nil {
...
func (c *Core) updateFTToken(senderAddress string, receiverAddress string, tokenInfo []contract.TokenInfo, tokenChainBlock []byte,
	...
	receiverPeerId, receiverDID, _ := util.ParseAddress(receiverAddress)
	b := block.InitBlock(tokenChainBlock, nil)
	...
	for i, ti := range tokenInfo {
		t := ti.Token
		wg.Add(1)
		go c.pinCheck(t, i, senderPeerId, receiverPeerId, results, &wg, nil)
	}
	wg.Wait()
	for i := range results {
		if results[i].Error != nil {
