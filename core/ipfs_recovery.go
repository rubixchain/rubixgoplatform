...
func NewIPFSRecoveryManager(core *Core) *IPFSRecoveryManager {
	...
	rm := &IPFSRecoveryManager{
		...
		isRecovering:   false,
		recoveryCount:  0,
		maxRecoveries:  10, // Maximum recovery attempts
		recoveryCtx:    ctx,
...
func (rm *IPFSRecoveryManager) startRecoveryMonitor() {
	...
	rm.log.Info("IPFS recovery monitor started")
}
...
func (rm *IPFSRecoveryManager) checkAndRecover() {
	...
	if rm.core.ipfsHealth.IsHealthy() {
		return
	}
...
func (rm *IPFSRecoveryManager) reinitializeIPFS() error {
	...
	rm.core.ipfsHealth.SetMaxConcurrency(rm.core.getConcurrencyLimit())

	// Reinitialize operations wrapper
	rm.core.ipfsOps = NewIPFSOperations(rm.core)

	// Update wallet IPFS reference
	if rm.core.w != nil {
		rm.core.w.SetupWallet(newShell, rm.core)
	}

	// Update peer manager IPFS reference
	if rm.core.pm != nil {
		rm.core.pm.UpdateIPFS(newShell)
	}

	// Update listener IPFS reference
	if rm.core.l != nil {
		rm.core.l.UpdateIPFS(newShell)
	}

	// Update pubsub IPFS reference
	if rm.core.ps != nil {
		rm.core.ps.UpdateIPFS(newShell)
	}
...
func (rm *IPFSRecoveryManager) restoreBootstrapPeers() error {
	...
	if len(bootstrapPeers) > 0 {
		_, err := rm.core.ipfsOps.BootstrapAdd(bootstrapPeers)
		if err != nil {
			return fmt.Errorf("failed to restore bootstrap peers: %w", err)
		}
