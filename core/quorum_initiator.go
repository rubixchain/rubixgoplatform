...
type ConsensusStatus struct {
	Credit          CreditScore
	QuorumSignature QuorumSignature // replace Credit struct with QuorumSignature struct
...
// to replace CreditScore by QuorumSignature wherever required
...
func (c *Core) initiateConsensus(cr *ConensusRequest, sc *contract.Contract, dc did.DIDCrypto) (*model.TransactionDetails, map[string]map[string]float64, *PledgeDetails, error) {
	...
	cs := ConsensusStatus{
		QuorumSignature: QuorumSignature{
			Signature: make([]block.BlockSignature, 0),
		},
	...
	publishingTxn := &model.PubSubTxnInfo{
		...
		TransactionValue: reqPledgeTokens,
	}
	...
	case RBTTransferMode:
		...
		if cr.SenderPeerID != cr.ReceiverPeerID {
			for _, t := range ti {
				...
				c.w.UnPin(tokenHash, constants.ProviderRole_PrevSender, sc.GetSenderDID())				
			}
	...
	case FTTransferMode:
		...
		go func() {
			...
			for _, tokeninfo := range ti {
				b := c.w.GetLatestTokenBlock(tokeninfo.Token, tokeninfo.TokenType)
				if b == nil {
					c.log.Error("unexpected error: failed to get the latest block height for token: %v", tokeninfo.Token)
					return
				}
				blockHeight, err := b.GetBlockNumber(tokeninfo.Token)
				if err != nil {
				...
				bid, errBlockID := b.GetBlockID(tokeninfo.Token)
				if errBlockID != nil {
					c.log.Error(fmt.Sprintf("unable to fetch current block id for Token %v, err: %v", tokeninfo.Token, err))
					return
				}
				...
				prevtokenIDTokenStateHash, errIpfsAdd := IpfsAddWithBackoff(c.ipfs, prevtokenIDTokenStateBuffer, ipfsnode.Pin(false), ipfsnode.OnlyHash(true))
				if errIpfsAdd != nil {
					c.log.Error(fmt.Sprintf("unable to get previous token state hash for token: %v, err: %v", tokeninfo.Token, errIpfsAdd))
					return
				}
				// send this exhausted hash to old quorums to unpledge
				for _, previousQuorumDID := range previousQuorumDIDs {
					...
					if previousQuorumInfo, ok = peerInfoCache[previousQuorumDID]; !ok {
						previousQuorumInfo, err = c.GetPeerDIDInfo(previousQuorumDID)
						if previousQuorumInfo.PeerID == "" || err != nil {
							c.log.Error(fmt.Sprintf("unable to get peerID for signer DID: %v. It is likely that either the DID is not created anywhere or %v", previousQuorumDID))
							return
						}
						peerInfoCache[previousQuorumDID] = previousQuorumInfo
					}
	...
	case PinningServiceMode:
		...
		// for _, t := range ti {
		// 	c.w.UnPin(t.Token, constants.ProviderRole_PrevSender, sc.GetSenderDID())
		// }
		// call ipfs repo gc after unpinnning
		// c.ipfsRepoGc()
		nbid, err := nb.GetBlockID(ti[0].Token)
...
func (c *Core) finishConsensus(id string, qt int, p *ipfsport.Peer, status bool, hash string, ss []byte, ps []byte) {
	...
	case 0:
		...
		if status {
			did := p.GetPeerDID()
			csig := block.BlockSignature{
				Signature: util.HexToStr(ps),
				DID:       did,
				Hash:      hash,
				SignType:  signType,
			}
			if cs.Result.SuccessCount < MinConsensusRequired {
				if _, ok := pd.PledgedTokens[did]; ok {
					cs.P[did] = p
					cs.QuorumSignature.Signature = append(cs.QuorumSignature.Signature, csig)
...
func (c *Core) pledgeQuorumToken(cr *ConensusRequest, sc *contract.Contract, tid string, dc did.DIDCrypto) (*block.Block, error) {
	...
	ti := sc.GetTransTokenInfo()
	Signature := make([]block.BlockSignature, 0)
	for _, csig := range cs.QuorumSignature.Signature {
		sign := block.BlockSignature{
	...
	if cr.Mode == SmartContractDeployMode {
		...
		signData, _, deployerPrivSign, err := sc.GetHashSig(bti.DeployerDID)
		if err != nil {
		...
		deployerSign := &block.BlockSignature{
			Signature: deployerPrivSign,
			DID:       bti.DeployerDID,
			Hash:      signData,
		...
		signData, _, executorPrivSign, err := sc.GetHashSig(bti.ExecutorDID)
		if err != nil {
		...
		executorSign := &block.BlockSignature{
			Signature: executorPrivSign,
			DID:       bti.ExecutorDID,
			Hash:      signData,
		...
		executorSignType := dc.GetSignType()
		executor_sign := &block.BlockSignature{
			Signature: executorPrivSign,
			DID:       bti.ExecutorDID,
			Hash:      signData,
			SignType:  executorSignType,
		...
		deployerSignType := dc.GetSignType()
		deployer_sign := &block.BlockSignature{
			Signature: deployerPrivSign,
			DID:       bti.DeployerDID,
			Hash:      signData,
			SignType:  deployerSignType,
		...
		senderdid := sc.GetSenderDID()
		signData, _, senderPrivSign, err := sc.GetHashSig(senderdid)
		if err != nil {
		...
		senderSign := &block.BlockSignature{
			Signature: senderPrivSign,
			DID:       senderdid,
			Hash:      signData,
