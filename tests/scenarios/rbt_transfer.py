...
def shuttle_transfer(config):
    ...
    for _, val in quorum_config.items():
        add_peer_details(val["peerId"], val["dids"]["did_quorum"], val["did_type"], server_port_A, grpc_port_A)

    quorum_config2 = load_from_config_file("./quorum_config2.json")
    for _, val in quorum_config2.items():
        add_peer_details(val["peerId"], val["dids"]["did_quorum"], val["did_type"], server_port_B, grpc_port_B)
