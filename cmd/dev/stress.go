...
func buildTransferReq(w *wallet.Wallet, cfg *config.Config, fromDID, toDID, tokenID string, epochOffset int64) (*wallet.PostConsensusPersistenceRequest, error) {
...
func buildTransferReqFromTok(cfg *config.Config, fromDID, toDID string, tok *models.Token, epochOffset int64) (*wallet.PostConsensusPersistenceRequest, *models.TransactionInfo, string, error) {
	tokenID := tok.TokenID
...
func RunStressTests(w *wallet.Wallet, cfg *config.Config, dids []string, didTokens map[string][]string) {
	fmt.Println("\n=== STRESS TESTS ===")
...
func runTransferFuzz(w *wallet.Wallet, cfg *config.Config, dids []string, availableTokens []string, currentOwners map[string]string) {
