...
func InitDIDQuorumLite(did string, baseDir string, pwd string) *DIDQuorumLite {
	...
	if _, err := ioutil.ReadDir(d.dir); err != nil {
		fmt.Printf("InitDIDQuorumLite: Directory does not exist or not readable: %s, error: %v\n", d.dir, err)
		return nil
	}
	...
	if d.pwd != "" {
		...
		if err != nil {
			fmt.Printf("InitDIDQuorumLite: Failed to read private key file: %s, error: %v\n", d.dir+PvtKeyFileName, err)
			fmt.Println("private key must be in wallet")
		} else {
			d.privKey, _, err = crypto.DecodeBIPKeyPair(d.pwd, privKey, nil)
			if err != nil {
				fmt.Printf("InitDIDQuorumLite: Failed to decode private key: %v\n", err)
				return nil
			}
		}
	}
	...
	if err != nil {
		fmt.Printf("InitDIDQuorumLite: Failed to read public key file: %s, error: %v\n", d.dir+PubKeyFileName, err)
		return nil
	}
	_, d.pubKey, err = crypto.DecodeBIPKeyPair("", nil, pubKey)
	if err != nil {
		fmt.Printf("InitDIDQuorumLite: Failed to decode public key: %v\n", err)
		return nil
	}

	fmt.Printf("InitDIDQuorumLite: Successfully initialized for DID: %s\n", did)
	return d
}
